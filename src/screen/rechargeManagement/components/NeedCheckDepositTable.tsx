...
import {
    addUserDeposit, getNeedCheckRecharges, handleEditData, updateDepositToIsChargedTrue
...
function NeedCheckDepositTable() {
  const columns = [
    {
      title: "id",
      dataIndex: "id",
  ...
  useEffect(() => {
    async function asyncGetRechargesData() {
      const result = await getNeedCheckRecharges();
      setdata(result);
    }
...
export default NeedCheckDepositTable;
